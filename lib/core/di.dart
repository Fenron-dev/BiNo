...
//        Definiert alle "singleton-artigen" Provider für DB, DAOs und Repositories.
// ABHÄNGIGKEITEN: database.dart, alle DAOs und Repositories.
...
// Das macht Unit- und Widget-Tests sauber und isolation.
...
///
/// keepAlive: true verhindert, dass Riverpod die DB schließt, wenn kein
/// Widget mehr auf den Provider hört. Die DB-Verbindung muss immer offen bleiben.
final databaseProvider = Provider<AppDatabase>((ref) {
  final db = AppDatabase();
  // Beim Dispose der ProviderScope (z. B. in Tests) wird die DB sauber geschlossen.
  ref.onDispose(db.close);
  return db;
}, name: 'databaseProvider');

/// Stellt den EntryDao aus der AppDatabase bereit.
///
/// WARUM ein eigener Provider statt direkt aus databaseProvider.entryDao?
/// Einzelne DAOs können in Tests unabhängig überschrieben werden,
/// ohne die gesamte Datenbank zu mocken.
final entryDaoProvider = Provider<EntryDao>((ref) {
  return ref.watch(databaseProvider).entryDao;
}, name: 'entryDaoProvider');

/// Stellt den TagDao aus der AppDatabase bereit.
final tagDaoProvider = Provider<TagDao>((ref) {
  return ref.watch(databaseProvider).tagDao;
}, name: 'tagDaoProvider');

/// Stellt den ContainerDao aus der AppDatabase bereit.
...
/// Stellt das EntryRepository bereit – Hauptkoordinator für Eintrags-Logik.
